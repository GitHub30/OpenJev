...
Qwen2.5-1.5B-Instruct (CPU) で冒頭の Stripe の例を投げた結果です。

```json
{
  "urgency":  {"type": "noul", "noul": 0.977},
  "spanish":  {"type": "noul", "noul": 0.014},
  "severity": {"type": "score", "score": 1.99, "confidence": 0.91,
               "probabilities": {"0": 0.00001, "1": 0.008, "2": 0.992}},
  "language": {"type": "choice", "choice": "english", "confidence": 0.97, "...": "13 択"}
}
```

緊急度・言語判定・深刻度は 1.5B でも問題なく、13 択 (2 桁ラベルあり) も正しく動きました。一方で「Stripe の接続に失敗」を `billing` (0.74) と `integrations` (0.26) に割ってしまうような微妙な区別は 1.5B では弱く、7B に上げるか `criteria` の説明を具体的にする必要があります。

Colab (A100) では Qwen2.5-7B-Instruct が自動で選ばれ、推論 → 評価/校正 → API サーバー + 公式 SDK → LoRA 微調整まで一通り動きます。
